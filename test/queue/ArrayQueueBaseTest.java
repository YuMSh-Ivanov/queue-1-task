...
public class ArrayQueueBaseTest<T extends QueueWrapper> {
    protected final Random random = new Random(3447982348967759878L);

    private void testEmpty(final T queue) {
        assertSize(0, queue);
    }

    private void testSingleton(final T queue) {
        assertSize(0, queue);
    ...
    private void testClear(final T queue) {
        assertSize(0, queue);
    ...
    protected void otherOperations(final List<Pair<T, LinkedList<Object>>> queues, final Pair<T, LinkedList<Object>> pair) {
    }

    private void moreQueues(final List<Pair<T, LinkedList<Object>>> queues, final Pair<T, LinkedList<Object>> pair) {
    ...
    private void testRandom(final Pair<T, LinkedList<Object>> initial, final double addFreq) {
        final List<Pair<T, LinkedList<Object>>> queues = new ArrayList<>();
        queues.add(initial);
        int ops = 0;
        for (int i = 0; i < 50_000; i++) {
            final Pair<T, LinkedList<Object>> pair = queues.get(random.nextInt(queues.size()));
        ...
        for (final Pair<T, LinkedList<Object>> pair : queues) {
            moreQueues(queues, pair);
            final int size = pair.second().size();
            assertSize(size, pair.first());
    ...
    private void assertSize(final int size, final T queue) {
        Assert.assertEquals("size() of " + queue, size, queue.size());
        Assert.assertEquals("isEmpty() of " + queue, size == 0, queue.isEmpty());
    }

    protected void test(final Supplier<T> queueActual, final Supplier<LinkedList<Object>> queueExpected) {
